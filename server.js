...
// - ARGO-V3.0 Instrument Configuration -
  ...
  try {
    ...
    if (text.startsWith("<")) {
      ...
      if (_alpacaConsecFails >= ALPACA_CIRCUIT_THRESHOLD && !_alpacaCircuitOpen) {
        ...
        if (RESEND_API_KEY && GMAIL_USER) sendResendEmail("ARGO-V3.0 ALERT - Alpaca API degraded",
...
async function runScan() {
  ...
  for (const { stock, price, bars, intradayBars, sectorResult, preMarket, newsArticles, analystData, eqScore } of stockData) {
    ...
    const entryWindowClosed = etHourNow >= 14.5; // entry cutoff 2:30pm ET — all trade types
    ...
    // All entries (including MR calls) cut at 2:30pm — 45min buffer before 3:15pm EOD close.
    // Evidence: 5/27 overnight hold (3:26pm entry), 5/28 two EOD losses, 6/8 QQQ -$103 (2:50pm entry).
    // MR calls previously had a 3:30pm exception — that exception is removed. 2:30pm is hard cutoff.
    const isMREntry = (callSetup.isMeanReversion || putSetup.isMeanReversion);
    const mrWindowOpen = etHourNow < 14.5; // all entries cut at 2:30pm ET (was 3:30pm)
    if (entryWindowClosed && !dryRunMode) {
      if (!isMREntry) {
        logEvent("filter", `[EOD-BLOCK] ${stock.ticker} entry window closed (after 2:30pm ET) — 45min buffer before 3:15pm EOD close`);
        continue;
      } else if (!mrWindowOpen) {
        logEvent("filter", `[EOD-BLOCK] ${stock.ticker} MR entry window closed (after 2:30pm ET) — all entries blocked`);
        continue;
      }
    }
...
// 8:00am ET — daily IVR seed refresh. Pulls fresh VIXY closes to advance _vixRolling
// with real daily closes (vs intraday tick accumulation during market hours).
// forceRefresh=true bypasses the length>=30 guard so it always runs.
// Uses same pattern as other crons: schedule fires at both 12:00 and 13:00 UTC to
// handle EDT/EST shift; internal ET hour/minute check ensures it fires exactly once.
cron.schedule("0 12,13 * * 1-5", async () => {
  const et = getETTime();
  if (et.getHours() !== 8 || et.getMinutes() !== 0) return;
  logEvent("scan", "[IVR] 8:00am ET — daily VIX history refresh starting...");
  try {
    const endDate   = new Date().toISOString().split("T")[0];
    const startDate = new Date(Date.now() - 380 * 86400000).toISOString().split("T")[0];
    let vixyBars = null;
    ...
    if (vixyBars && vixyBars.length > 60) {
      ...
      if (seedMax - seedMin >= 10) {
        ...
        state._ivRank = seedP95 > seedP5
          ...
          : 50;
        state._ivEnv  = state._ivRank >= 70 ? "high" : state._ivRank >= 50 ? "elevated" : state._ivRank >= 30 ? "normal" : "low";
        logEvent("scan", `[IVR] Daily refresh complete — ${state._vixRolling.length} days | P5-P95:[${seedP5.toFixed(1)}-${seedP95.toFixed(1)}] | IVR:${state._ivRank} (${state._ivEnv})`);
        markDirty();
      } else {
        logEvent("warn", `[IVR] Daily refresh skipped — range too narrow (${seedMin.toFixed(1)}-${seedMax.toFixed(1)})`);
      }
    } else {
      logEvent("warn", "[IVR] Daily refresh — insufficient VIXY data returned");
    }
  } catch(e) {
    logEvent("warn", `[IVR] Daily refresh failed: ${e.message}`);
  }
});
...
app.post("/api/reset-account", requireSecret, async (req, res) => {
  ...
  state.peakCash        = null; // null forces formula to MONTHLY_BUDGET — prevents stale Redis value causing false CRITICAL drawdown
