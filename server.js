...
const systemPrompt = `Post-market analyst for ARGO-V1.5. Return ONLY valid JSON — no markdown. {"overnightRisk":"low"|"medium"|"high","holdRecommendations":{},"tomorrowBias":"bullish"|"bearish"|"neutral","catalystsTomorrow":[],"reasoning":"1-2 sentences"} holdRecommendations: {ticker: "HOLD"|"MONITOR"|"EXIT_AT_OPEN"} for each open position.`;
...
const userPrompt = `${scanType} assessment — ${new Date().toLocaleString('en-US', {timeZone:'America/New_York'})} ET VIX: ${mktStatus.vix || state.vix} | SPY close: ${mktStatus.spy?.price || '--'} Open positions: ${JSON.stringify(positions)} Recent headlines: ${(headlines.headlines || []).slice(0,5).join(' | ')} What is the overnight risk and tomorrow's bias?`;
...
const userPrompt = `Re-entry check: ${stock.ticker} ${optionType.toUpperCase()} Closed at a loss ${hrsSinceLoss.toFixed(1)}h ago (reason: ${recentLoss.reason}, P&L: ${recentLoss.pnlPct}%) Current: RSI ${stock.rsi} | MACD ${stock.macd} | Momentum ${stock.momentum} Entry macro was: ${recentLoss.agentSignal} | Current macro: ${(state._agentMacro||{}).signal||'neutral'} Score now: ${score}/100 | Top reasons: ${reasons.slice(0,3).join('; ')} Has the thesis genuinely changed since the loss? Approve re-entry?`;
...
const systemPrompt = `Options pre-entry analyst. Return JSON only: {"approved":true|false,"confidence":"high"|"medium"|"low","reason":"one sentence"}`;
const userPrompt = `Pre-entry check: ${stock.ticker} ${optionType.toUpperCase()} RSI: ${stock.rsi} | MACD: ${stock.macd} | Momentum: ${stock.momentum} Score: ${score}/100 | Top reasons: ${reasons.slice(0,4).join('; ')} Macro: ${(state._agentMacro||{}).signal||'neutral'} (${(state._agentMacro||{}).confidence||'unknown'}) VIX: ${state.vix} Should ARGO-V1.5 enter this ${optionType} position?`;
...
try {
if (call.name === “getQuote”)          results[call.name + ‘*’ + call.input.ticker] = await agentTool_getQuote(call.input.ticker);
else if (call.name === “getLiveSignals”)   results[call.name + ’*’ + call.input.ticker] = await agentTool_getLiveSignals(call.input.ticker);
...
1. POSITION REVIEW (1 sentence per position): For each open position, check its current status and say whether to hold, watch, or exit and why.
1. OPPORTUNITIES (1-2 sentences): Any mean reversion call opportunities forming? Any positions approaching targets?
...
`<div style="font-family:monospace;background:#07101f;color:#ff5555;padding:20px"> <h2>⚠ ARGO-V1.5 Scanner Error</h2> <p>Consecutive scan failures: <strong>${state._scanFailures}</strong></p> <p>Last error: ${e.message}</p> <p>Time: ${new Date().toISOString()}</p> <p>Open positions: ${state.positions.length}</p> <p>Cash: $${state.cash}</p> <p><strong>Check Railway logs immediately.</strong></p> </div>`
);
...
body: JSON.stringify({
from:    “ARGO-V1.5 [onboarding@resend.dev](mailto:onboarding@resend.dev)”,
to:      [GMAIL_USER],
subject,
html,
}),
...
`<p>ARGO-V1.5 has not scanned in ${minsSinceLastScan.toFixed(0)} minutes during market hours.</p> <p>Last scan: ${state.lastScan || "unknown"}</p> <p>Check Railway logs immediately.</p>`
);
}
});
...
`<div style="font-family:monospace;background:#07101f;color:#00ff88;padding:20px;border-radius:8px"> <h2>✅ ARGO-V1.5 Email Working</h2> <p style="color:#cce8ff">If you received this, Resend is configured correctly.</p> <p style="color:#336688">Recipient: ${GMAIL_USER}</p> <p style="color:#336688">Sent at: ${new Date().toISOString()}</p> </div>`
);
